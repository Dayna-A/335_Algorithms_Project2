...
std::unique_ptr<FoodVector> filter_food_vector(const FoodVector& source,
	...
	FoodVector todo = source;
	std::unique_ptr<FoodVector> result(new FoodVector);

	for (size_t i = 0; i < todo.size(); i++) {
		//  std::unique_ptr<Food> copy = todo[i];
		if (todo[i]->kcal() >= min_kcal && todo[i]->kcal() <= max_kcal && todo[i]->kcal() != 0 && result->size() < total_size) {
			result->push_back(todo[i]);
		}
	}
...
std::unique_ptr<FoodVector> greedy_max_protein(const FoodVector& foods,
	...
	while (!todo.empty()) {
		...
		for (int i = 0; i < todo.size(); i++) {
			if (todo[i]->kcal() > todo[max_index]->kcal())
				max_index = i;
		}

		if ((todo[max_index]->kcal() + result_cal) <= total_kcal) {
			result->push_back(todo[max_index]);
			result_cal += todo[max_index]->kcal();
		}
